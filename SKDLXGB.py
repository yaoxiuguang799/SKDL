...
# ----------------------------- 特征工程模块 ----------------------------- #
class XGBTFeatureGenerator:
    def __init__(self, model_save_path, device: str = 'auto'):
        ...
        if os.path.exists(self.model_save_path):
            self.xgb_model = joblib.load(self.model_save_path)
    ...
    def auto_tune(self, X: np.ndarray, y: np.ndarray, n_trials: int = 20):
        ...
        def objective(trial):
            ...
            leaves_train, leaves_val = self._train_xgb(X_train, y_train, X_val, params)
            emb_dim = trial.suggest_int('emb_dim', 4, 16)
            model = SkdlXGB(X_train.shape[1], params['n_estimators'], max(np.max(leaves_train)+1, 10), emb_dim)
            score = self._evaluate_model(model, leaves_train, y_train, leaves_val, y_val)
            return score

        study = optuna.create_study(direction='minimize')
        study.optimize(objective, n_trials=n_trials)
        self.optimal_params = study.best_params
        self.train_xgb(X, y, None, self.optimal_params)


    def train_xgb(self, X_train, y_train, X_val=None, params=None):
        if params is not None:
            self.optimal_params = params
        X_scaled = self.scaler.fit_transform(X_train)
        xgb_train = xgb.DMatrix(X_scaled, label=y_train)

        if self.xgb_model != None:
            return None, None

        self.xgb_model = xgb.train(params, xgb_train)
 
        leaves_train = self.xgb_model.predict(xgb_train, pred_leaf=True)
        if X_val is not None:
            X_val_scaled = self.scaler.transform(X_val)
            dval = xgb.DMatrix(X_val_scaled)
            leaves_val = self.xgb_model.predict(dval, pred_leaf=True)
            return leaves_train, leaves_val
        return leaves_train, None

    def _create_model(self, num_numerical, n_estimators, num_leaves):
        return SkdlXGB(num_numerical=num_numerical, n_estimators=n_estimators, num_leaves=num_leaves).to(device)
    ...
    def generate_features(self, X: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        dval = xgb.DMatrix(X_scaled)
        leaves = self.xgb_model.predict(dval, pred_leaf=True)
        return torch.FloatTensor(X_scaled).to(self.device), torch.LongTensor(leaves).to(self.device)

# ----------------------------- 深度网络模块 ----------------------------- #
class SkdlXGB(nn.Module):
    def __init__(self, num_numerical, n_estimators, num_leaves, emb_dim=8, use_attention=False):
        super().__init__()
...
def main():
    ...
    for Dis in DistanceDensity:
        ...
        test_result_save_path = os.path.join(wp_path, 'result', f'deepXGB_{channel}_{cmask}_{Dis}_test.csv')
        Val_result_save_path = os.path.join(wp_path, 'result', f'deepXGB_{channel}_{cmask}_{Dis}_cv10.csv')
        ...
        feature_gen = XGBTFeatureGenerator(model_save_path=os.path.join(wp_path, 'model', f'deepXGB_{channel}_{cmask}_{Dis}.joplib'))
        feature_gen.train_xgb(data_processor.X_train, data_processor.y_train, X_val=None, params=best_params)
        ...
        train_loader, val_loader = create_data_loaders(X_num, X_emb, data_processor.y_train, batch_size=config0['batch_size'])
        

        model = SkdlXGB(
            num_numerical=X_num.shape[1],
            n_estimators=feature_gen.optimal_params['n_estimators'],
            num_leaves=np.max(X_emb.cpu().numpy())+1,
        ).to(device)

        trainer = SkdlXBGTrainer(model_save_path=os.path.join(wp_path, 'model', f'deepXGB_best_{channel}_{cmask}_{Dis}.pth'),
                                model=model,
                                config=config0
                                )
    ...
    np.savetxt(os.path.join(wp_path, 'model', 'elapsed', f'deepXGB_{channel}_{cmask}_elapsed.csv'),elapsed_list, delimiter=',',fmt='%s')
    print('All done!')
