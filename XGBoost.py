...
class XGBoostBayesTrainer:
    ...
    def __init__(self, data_processor: DataProcessor, model_save_path: str, result_save_path: str, param_bounds: dict = None, random_state: int = 7):
        ...
        self.param_bounds = param_bounds or {
            ...
            'min_child_weight': (0, 20.0)
        }
        self.model = None
        self.load_model()

    def _build_model(self, params: dict) -> XGBRegressor:
        """基于参数构建XGBoost模型"""
        return XGBRegressor(
            n_estimators=math.ceil(params['n_estimators']),
            max_depth=math.ceil(params['max_depth']),
            learning_rate=params['learning_rate'],
            min_child_weight=math.ceil(params['min_child_weight']),
            random_state=self.random_state
        )

    def bayes_opt_objective(self, n_estimators, max_depth, learning_rate, min_child_weight):
        params = {
    ...
    def _save_results_and_evaluate(self, outputs: np.ndarray, save_path: str, description: str):
        ...
        R, BIAS, MAE, MRE, RMSE, KGE = accuracyEvaluate(outputs[:, -2], outputs[:, -1], True)
        print(f"{description} Evaluation -> R: {R:.4f}, BIAS: {BIAS:.2f}, RMSE: {RMSE:.4f}, KGE: {KGE:.4f}")
...
if __name__ == '__main__':
    ...
    for Dis in DistanceDensity:
        ...
        trainer = XGBoostBayesTrainer(
            data_processor=data_processor,
            model_save_path=os.path.join(wp_path, 'model', f'XGBoost_best_{channel}_{cmask}_{Dis}.joblib'),
            result_save_path=os.path.join(wp_path, 'result', f'XGBoost_{channel}_{cmask}_{Dis}_test.csv')
        )
    ...
    np.savetxt(os.path.join(wp_path, 'model', 'elapsed', f'XGBoost_{channel}_{cmask}_elapsed.csv'),elapsed_list, delimiter=',',fmt='%s')
