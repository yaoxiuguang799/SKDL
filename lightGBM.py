...
class LightgbmBayesTrainer:
    ...
    def __init__(self, data_processor: DataProcessor, model_save_path: str, result_save_path: str, param_bounds: dict = None, random_state: int = 7):
        ...
        self.param_bounds = param_bounds or {
            ...
            'num_leaves': (30.0, 100.0)
        }
        self.model = None
        self.load_model()

    def _build_model(self, params: dict) -> LGBMRegressor:
        """基于参数构建Lightgbm模型"""
    ...
    def bayes_opt_objective(self, n_estimators, max_depth, learning_rate, num_leaves):
        params = {
...
if __name__ == '__main__':
    ...
    for Dis in DistanceDensity:
        ...
        trainer = LightgbmBayesTrainer(
            data_processor=data_processor,
            model_save_path=os.path.join(wp_path, 'model', f'Lightgbm_best_{channel}_{cmask}_{Dis}.joblib'),
            result_save_path=os.path.join(wp_path, 'result', f'Lightgbm_{channel}_{cmask}_{Dis}_test.csv')
        )
    ...
    np.savetxt(os.path.join(wp_path, 'model', 'elapsed', f'Lightgbm_{channel}_{cmask}_elapsed.csv'),elapsed_list, delimiter=',',fmt='%s')
