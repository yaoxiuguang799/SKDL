...
class GBMTFeatureGenerator:
    ...
    def _create_model(self, num_numerical, n_estimators, num_leaves):
        return SkdlGBM(num_numerical=num_numerical, n_estimators=n_estimators, num_leaves=num_leaves).to(device)
...
def main():
    ...
    for Dis in DistanceDensity:
        ...
        trainer = SkdlGBMTrainer(model_save_path=os.path.join(wp_path, 'model', f'deepGBM_best_{channel}_{cmask}_{Dis}.pth'),
                                model=model,
                                config=config0
                                )
